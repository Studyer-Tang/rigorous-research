...
The packet includes the question, contract, claim/assumption/check text, source citations, and task/release gaps. It does not load source PDFs, datasets, raw artifacts, or evidence-file contents. Research text inside the packet can still be confidential; `prepare` sends nothing. Sharing it through a chat or API is a separate action initiated by the user.
...
Inspect both `execution_status` and the nested certificate. `CHECKED` means the backend completed, which can yield a proof, a counterexample, or an inconclusive result. A timeout or parsing error yields no certificate. A returned proof does not validate the model's translation, quantifiers, omitted assumptions, or connection to the research claim. No task is marked complete and no evidence or case verdict is written automatically. The existing receipt gate for established identities is not extended to accept bound or counterexample outputs by relabeling them.
