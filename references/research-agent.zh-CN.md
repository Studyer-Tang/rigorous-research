...
1.13 版加入 `route`：用固定 ID 保存研究路线的命题、方法、障碍、下一项检验和证据编号。
最多 32 条路线的最新版本持续进入上下文，不受最近 20 次动作窗口影响；旧版本可按动作编号
检索。`supported` 仅表示模型对路线的判断，不关闭证明义务。旧研究库自动添加路线索引，保留原始历史。

通用不等式工具包括 `inequality_search`（精确可行反例）、`polynomial_sos`（自动二次平方和
或模型提供的条件平方和）、`polynomial_amgm`（模型提供的 AM-GM 证明）。每条假设均表示一个
多项式非负。独立标准库检查器重新计算恒等式和反例，不调用 SymPy。详见
[证明接口](polynomial-proofs.md)与[中文实跑报告](../examples/polynomial-routes/REPORT.zh-CN.md)。
二次证明之外的分解仍需模型寻找；统计应用中的抽样、识别与适用条件不因代数核验而成立。
