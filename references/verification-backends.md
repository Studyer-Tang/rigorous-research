...
`math_backend.py sympy-bound` checks a one-variable polynomial inequality on a closed rational interval using exact Bernstein coefficients and bounded subdivision. Nonnegative coefficients on every subinterval establish the bound; a negative coefficient only requests subdivision. Refutation requires an exact negative polynomial value at an admissible point. The certificate records interval coverage, rational coefficients, unresolved intervals, and witnesses. This is a sufficient method: a true inequality can remain inconclusive at the subdivision budget. Inputs with variable denominators are rejected even if cancellation would yield a polynomial. Bound certificates are separate from the existing established-identity receipt gate.
...
`math_backend.py lean-check` records compiler output and scans for `sorry`, `admit`, and user-declared axioms. All compilation records remain diagnostic: a clean scan does not establish target theorem identity or transitive imported axiom closure. Lean is optional. If it is unavailable, report that the formal check was not run; never imitate a compiler transcript.

Wrap independently established SymPy output with `research_seal.py wrap-receipt`. Lean records can be bound for provenance but cannot pass strict established-receipt validation. The receipt binds claim inputs, certificate outputs, dependency locks, backend name and version, command, return code, and semantic domain. Release validation reopens the backend output and independently recomputes its mathematical content; self-written text labeled “exact computation” is insufficient.
