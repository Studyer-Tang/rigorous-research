...
**1.15 版可核验有限耦合最优解的熵界。** [本轮 Frankl 研究](examples/frankl-coupling/REPORT.zh-CN.md) 用互不重叠的严格界排除“逆权重越强、熵就越高”的一般路线，并研究接近一半频率的集合族。检查器验证最优性误差和双方边缘约束，不把求解器残差小当成证明。完整 Frankl 猜想仍未解决。

**1.14 版加入精确熵比较，并开始攻研完整 Frankl 猜想。** 有理有限分布的熵不等式由独立检查器化为整数比较，耦合假设单独保留。[本轮研究记录](examples/frankl-frontier/REPORT.zh-CN.md) 包含最新原始文献的限制、错误证明路线的精确反例与乘积集合族的约束。完整猜想仍未解决，工具升级和局部结果不视为最终目标完成。

**1.13 版加入通用多项式证明工具与持久化研究路线。** Agent 可自动寻找有理系数二次多项式的平方和证明，核验模型提交的带条件平方和与 AM-GM 证明，并搜索精确可行反例。独立标准库检查器复核数学内容。[柯西不等式、加权方差和 Motzkin 实跑](examples/polynomial-routes/REPORT.zh-CN.md) 保留失败尝试和方法切换；这些是已知基准，不代表解决了公开未解猜想或证明了模型能力提升。

**1.12 版加入原生整数研究和无限参数族检查。** [本轮多路线实战](examples/erdos-frontier/README.md) 否定了小窗口辅助猜想，独立核验 55 个参数族，并明确保留 6 个同余类缺口；Agent 上下文现在持续显示任务与证明义务。原始猜想仍未解决。
