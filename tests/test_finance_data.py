...
class FinanceDataTests(unittest.TestCase):
    ...
    def snapshot(self, raw: bytes, as_of: str):
        return finance.create_snapshot(
            raw, self.root, provider="Test Exchange", dataset="daily prices", url="https://data.test/prices",
            query={"symbol": "ABC"}, as_of=as_of, revision_policy="append-only fixture",
            schema="date,close", units="USD", timezone="UTC", calendar="weekdays",
            identifier_system="fixture symbol", adjustment_policy="unadjusted", license_text="test only",
            extension="csv", retrieved_at="2026-01-01T00:00:00+00:00",
        )
